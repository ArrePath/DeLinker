            ...
            for i, mol in enumerate(mols):
                ...
                if i == 1:
                    ...
                    if len(dummy_atom_idxs) != 2:
                        ...
                        path_length = (
                            len(
                                Chem.rdmolops.GetShortestPath(
                                    mol, dummy_atom_idxs[0], dummy_atom_idxs[1]
                                )
                            )
                            - 2
                        )
...
def get_linker(full_mol, clean_frag, starting_point):
    ...
    if len(matches) > 0:
        ...
        for match in matches:
            ...
            # Get linker atoms
            linker_atoms = list(
                set(list(range(full_mol.GetNumHeavyAtoms()))).difference(match)
            )
            linker_bonds = []
            ...
            for idx_to_delete in sorted(match, reverse=True):
                nei = [
                    x.GetIdx()
                    for x in mol_rw.GetAtomWithIdx(idx_to_delete).GetNeighbors()
                ]
                intersect = set(nei).intersection(set(linker_atoms))
                if len(intersect) == 1:
                    linker_bonds.append(
                        mol_rw.GetBondBetweenAtoms(
                            idx_to_delete, list(intersect)[0]
                        ).GetIdx()
                    )
                    ...
                    for idx_nei in list(intersect):
                        linker_bonds.append(
                            mol_rw.GetBondBetweenAtoms(idx_to_delete, idx_nei).GetIdx()
                        )
                        atoms_joined_to_linker.append(idx_to_delete)
            ...
            if linker.GetNumHeavyAtoms() == linker_len:
                ...
                if len(Chem.rdmolops.GetMolFrags(frags)) == 2:
                    ...
                    if (
                        len(Chem.rdmolops.GetMolFrags(linker)) == 1
                        and len(linker_bonds) == 2
                    ):
                        ...
                        if len(Chem.rdmolops.GetMolFrags(linker_to_return)) > 1:
                            for frag in Chem.MolToSmiles(linker_to_return).split("."):
                                if (
                                    Chem.MolFromSmiles(frag).GetNumHeavyAtoms()
                                    == linker_len
                                ):
                                    return frag
                        return Chem.MolToSmiles(
                            Chem.MolFromSmiles(Chem.MolToSmiles(linker_to_return))
                        )
    ...
    # If no possibilities, process failed
        ...
        print(
            "FAIL:",
            Chem.MolToSmiles(full_mol),
            Chem.MolToSmiles(clean_frag),
            starting_point,
        )
        return ""
...
def compute_distance_and_angle(mol, smi_linker, smi_frags):
    try:
        ...
        for frag_match, linker_match in product(frags_matches, linker_matches):
            ...
            f_match = [
                idx
                for num, idx in enumerate(frag_match)
                if frags.GetAtomWithIdx(num).GetAtomicNum() != 0
            ]
            l_match = [
                idx
                for num, idx in enumerate(linker_match)
                if linker.GetAtomWithIdx(num).GetAtomicNum() != 0 and idx not in f_match
            ]
        ...
        sub_idx += [
            idx
            for num, idx in enumerate(linker_match)
            if linker.GetAtomWithIdx(num).GetAtomicNum() != 0 and idx not in sub_idx
        ]
...
def compute_distance_and_angle_dataset(
    fragmentations, path_to_conformers, dataset, verbose=False
):
    ...
    for count, mol in enumerate(conformers):
        ...
        if mol is not None:
            mol_name = (
                mol.GetProp("_Name") if dataset == "ZINC" else Chem.MolToSmiles(mol)
            )
            if mol_name in dataset_dict:
                # Loop over all fragmentations of this mol
                for fragments in dataset_dict[mol_name]:
                    dist, ang = compute_distance_and_angle(
                        mol, fragments[0], fragments[1]
                    )
                    if dist and ang:
...
def compute_distance_and_angle_dataset_alt(
    fragmentations, path_to_conformers, verbose=False
):
    # Load conformers
    conformers = Chem.SDMolSupplier(path_to_conformers)
    # Convert dataset to dictionary
...
def plot_hist(results, prop_name, labels=None, alpha_val=0.5):
    ...
    for i, res in enumerate(results):
        if not labels:
            dens, _, _ = plt.hist(
                np.array(res).flatten(), bins=bins, density=True, alpha=alpha_val
            )
        elif labels:
            dens, _, _ = plt.hist(
                np.array(res).flatten(),
                bins=bins,
                density=True,
                alpha=alpha_val,
                label=labels[i],
    ...
    for ring in ssr:
        for atom_idx in ring:
            for bond in linker.GetAtomWithIdx(atom_idx).GetBonds():
                if (
                    bond.GetBondType() == 2
                    and bond.GetBeginAtomIdx() in ring
                    and bond.GetEndAtomIdx() in ring
                ):
                    check = False
    return check
    ...
    for processed, res in enumerate(results):
        ...
        for m in res:
            ...
            clean_frag = Chem.RemoveHs(
                AllChem.ReplaceSubstructs(
                    Chem.MolFromSmiles(m[0]), du, Chem.MolFromSmiles("[H]"), True
                )[0]
            )
...
def check_2d_filters(toks, pains_smarts, count=0, verbose=False):
    ...
    du = Chem.MolFromSmiles("*")
    clean_frag = Chem.RemoveHs(
        AllChem.ReplaceSubstructs(frags, du, Chem.MolFromSmiles("[H]"), True)[0]
    )
...
def check_2d_filters_dataset(
    fragmentations, n_cores=1, pains_smarts_loc="./wehi_pains.csv"
):
    # Load pains filters
    with open(pains_smarts_loc, "r") as f:
        pains_smarts = [
            Chem.MolFromSmarts(line[0], mergeHs=True) for line in csv.reader(f)
        ]

    with Parallel(n_jobs=n_cores, backend="multiprocessing") as parallel:
        results = parallel(
            delayed(check_2d_filters)(toks, pains_smarts, count, True)
            for count, toks in enumerate(fragmentations)
        )

    fragmentations_filtered = [
        toks for toks, res in zip(fragmentations, results) if res
    ]
...
def calc_filters_2d_dataset(results, pains_smarts_loc, n_cores=1):
    ...
    with Parallel(n_jobs=n_cores, backend="multiprocessing") as parallel:
        filters_2d = parallel(
            delayed(calc_2d_filters)([toks[2], toks[4], toks[1]], pains_smarts)
            for toks in results
        )
            ...
            if (
                DataStructs.TanimotoSimilarity(
                    AllChem.GetMorganFingerprintAsBitVect(
                        Chem.MolFromSmiles(m[0]), 2, 2048
                    ),
                    AllChem.GetMorganFingerprintAsBitVect(
                        Chem.MolFromSmiles(m[2]), 2, 2048
                    ),
                )
                == 1
            ):
...
def recovered_by_smi_canon(results):
    ...
    for res in results:
        ...
        for m in res:
            ...
            if Chem.MolToSmiles(
                Chem.RemoveHs(Chem.MolFromSmiles(m[0]))
            ) == Chem.MolToSmiles(Chem.RemoveHs(Chem.MolFromSmiles(m[2]))):
                recovered += 1
                break
    return recovered / total
...
def average_linker_length(results):
    ...
    for res in results:
        for m in res:
            linker_length = (
                Chem.MolFromSmiles(m[1]).GetNumHeavyAtoms()
                - Chem.MolFromSmiles(m[0]).GetNumHeavyAtoms()
            )
...
def join_frag_linker(linker, st_pt, random_join=True):
    if linker == "":
        ...
        return Chem.MolToSmiles(
            Chem.RemoveHs(
                AllChem.ReplaceSubstructs(
                    Chem.MolFromSmiles(st_pt), du, Chem.MolFromSmiles("[H]"), True
                )[0]
            )
        ).split(".")[0]
    ...
    if random_join:
        ...
        final_mol = sorted(
            Chem.MolToSmiles(final_mol).split("."), key=lambda x: len(x), reverse=True
        )[0]
        return final_mol
        ...
        for st_pt_exit_pts in [st_pt_exit_points, st_pt_exit_points[::-1]]:
            ...
            final_mol = combo_rw.GetMol()
            final_mol = sorted(
                Chem.MolToSmiles(final_mol).split("."),
                key=lambda x: len(x),
                reverse=True,
            )[0]
...
def SC_RDKit_frag_mol(gen_mol, ref_mol, start_pt):
    try:
        ...
        clean_frag = Chem.RemoveHs(
            AllChem.ReplaceSubstructs(
                Chem.MolFromSmiles(start_pt), du, Chem.MolFromSmiles("[H]"), True
            )[0]
        )
...
def SC_RDKit_frag_scores(gen_mols):
    return [
        SC_RDKit_frag_mol(gen_mol, ref_mol, frag_smi)
        for (gen_mol, ref_mol, frag_smi) in gen_mols
    ]
...
def rmsd_frag_scores(gen_mols):
    return [
        rmsd_frag_mol(gen_mol, ref_mol, start_pt)
        for (gen_mol, ref_mol, start_pt) in gen_mols
    ]
