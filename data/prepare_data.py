...
def preprocess(raw_data, dataset, name, test=False):
    ...
    processed_data = []
    total = len(raw_data)
    for i, (smi_mol, smi_frags, smi_link, abs_dist) in enumerate(
        [
            (mol["smi_mol"], mol["smi_frags"], mol["smi_linker"], mol["abs_dist"])
            for mol in raw_data
        ]
    ):
        if test:
            smi_mol = smi_frags
            smi_link = ""
        (mol_out, mol_in), nodes_to_keep, exit_points = align_mol_to_frags(
            smi_mol, smi_link, smi_frags
        )
        if mol_out == []:
            continue
