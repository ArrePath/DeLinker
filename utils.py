...
def graph_to_adj_mat(
    graph, max_n_vertices, num_edge_types, tie_fwd_bkwd=True, considering_edge_type=True
):
    if considering_edge_type:
...
def to_graph_mol(mol, dataset):
    ...
    for bond in mol.GetBonds():
        if (
            mol.GetAtomWithIdx(bond.GetBeginAtomIdx()).GetAtomicNum() == 0
            or mol.GetAtomWithIdx(bond.GetEndAtomIdx()).GetAtomicNum() == 0
        ):
            continue
        else:
            edges.append(
...
class ThreadedIterator:
    ...
    def worker(self, original_iterator):
        for element in original_iterator:
            assert (
                element is not None
            ), "By convention, iterator elements much not be None"
